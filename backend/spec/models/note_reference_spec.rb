    ...
    context 'reference_objs' do
      ...
      it '配列以外の型（文字列）登録不可' do
        expect { note_reference.update!(reference_objs: 'not an array', book:) }.to(raise_error do |error|
          expect(error).to be_a(ActiveRecord::RecordInvalid)
          expect(error.message).to eq('参照先は{ :likes=>Integer, :title=>String, :url=>String, :userProfileImg=>String }の配列にしてください')
        end)
      end
      ...
      it 'ハッシュのキーlikes, title, url, userProfileImgが揃ってないと登録不可' do
      ...
      it 'ハッシュの値が型と異なる値登録不可' do
        invalid_obj = {
          ...
          'userProfileImg' => 'http://example.com/image.jpg'
        }
