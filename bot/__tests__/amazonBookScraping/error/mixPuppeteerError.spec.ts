...
import { PuppeteerError } from '@/lib/errors'
...
jest.mock('@google-cloud/firestore', () => {
  const docsTemplateObject = {
    data: jest.fn().mockReturnValue({
      ...
      productUrls: ['https://www.amazon.co.jp/dp/xxxxxxxxx'],
      referenceObj: {
        likes: 28,
...
describe('crawling', () => {
  ...
  const puppeteerErrorRes = new PuppeteerError('PuppeteerError occurred', 'https://www.amazon.co.jp/dp/xxxxxxxxx')
  ...
  describe('再起処理', () => {
    ...
    describe('書籍情報取得が成功・NotBookErrorが混合する場合', () => {
      const getBookInfoSpied = jest.spyOn(bookInfoModule, 'getBookInfo')
      getBookInfoSpied.mockImplementationOnce((url: string) => {
      ...
      getBookInfoSpied.mockImplementationOnce((url: string) => {
        return Promise.resolve(puppeteerErrorRes)
      })
      getBookInfoSpied.mockImplementationOnce((url: string) => {
        return Promise.resolve({
          asin: 'B09QQPFB2W',
      ...
      it('エラー内容をFirestoreに保存するも処理は止まらず、2回再起処理が行われる', async () => {
        ...
        expect(consoleErrorSpied).toHaveBeenCalledWith('PuppeteerError occurred')
